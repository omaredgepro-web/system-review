...
async function uploadReadyDistributedCsvToSupabase() {
  ...
  const btn = document.getElementById('btn-upload-ready-distributed');
  btn.innerText = 'جاري الرفع...'; btn.disabled = true;
  ...
  try {
    ...
    if (error) { alert('خطأ أثناء الرفع: ' + error.message); }
      ...
      alert(`تم رفع ${cleanData.length} طلب "جاهز" بنجاح، زي ما هو في الملف تمامًا!`);
      await loadData();
      switchTab('dashboard');
    }
  } catch (err) { alert('خطأ: ' + err.message); }
  finally { btn.innerText = '📥 رفع وتوزيع طلبات جاهزة (كما هي في الملف)'; btn.disabled = false; }
}
